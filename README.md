...
tcpkiller is a sneaky way to kill network connections. Any targeted system will appear to work - DNS will resolve, ICMP's will go through, and the network card will be able to connect to the network, but no TCP connections will be able to by sustained. The current implementation is on the noiser side to ensure effectiveness, but a stealth option is on the way. 
